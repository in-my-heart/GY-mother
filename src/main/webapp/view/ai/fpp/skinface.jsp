...
var prefix = "<%=basePath%>";
 ...
 function load() {
	$('#exampleTable').bootstrapTable({
				...
				url : prefix + "fpp/listFPPFace", // 服务器数据的加载地址
				iconSize : 'outline',
				...
				columns : [
					...
					},
					{
						...
						formatter : function(value, row, index) {
							...
							var d = '<a class="btn btn-warning btn-sm ' + s_edit_h + '" " title="删除"  onclick="remove(\''+ row.fppId+'\')"><i class="fa fa-remove"></i></a> ';
							return d;
						}
					} ]
			});
		...
		layer.confirm("确认要删除选中的'" + rows.length + "'条数据吗?",{btn : [ '确定', '取消' ]
			...
			$.each(rows, function(i, row) {
					...
					$.ajax({
						...
						url : prefix + 'fpp/batchRemoveFPPFace',
						success : function(r) {
							if (r.code == 0) {
